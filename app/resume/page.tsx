...
const featuredProjects = [
  ...
  },
  {
    ...
    github: "",
    tagline: "Animated gauge dashboards surfacing **entry/exit market-timing signals** generated by **ZULFIQODE, a Quant Algo that predicts Global Financial Crisis**",
    bullets: [
      ...
      "Built the frontend in vanilla HTML/CSS/JavaScript with Chart.js, and engineered a custom spring-damper physics simulation to drive gauge needle animation with natural accelerate/decelerate motion in place of linear CSS transitions",
      "Engineered Node.js serverless functions on Vercel, consolidating multiple per-ticker API endpoints into a single parameterized function to operate within Vercel's Hobby-plan limits, with server-side API key handling for external market data and news APIs",
...
async function downloadPDF() {
  ...
  featuredProjects.forEach(p => {
    ...
    if ("tagline" in p && p.tagline) {
      style(8, false, AMBER);
      doc.setFont("helvetica", "italic");
      writeWrapped(p.tagline.replace(/\*\*(.+?)\*\*/g, "$1"), M, CW, 3.8);
    }
  ...
  const children = [
    ...
    ...featuredProjects.flatMap((p) => [
      ...
      ...("tagline" in p && p.tagline
        ? [new Paragraph({ children: [new TextRun({ text: p.tagline.replace(/\*\*(.+?)\*\*/g, "$1"), size: 18, color: "b45309", italics: true })] })]
        : []),
      ...p.bullets.map(
        (b) =>
          new Paragraph({
          ...
          <div className="space-y-4">
            {featuredProjects.map((p, i) => (
              <div key={i} className="text-sm">
                ...
                {"tagline" in p && p.tagline && (
                  <p className="text-xs italic text-amber-800 mt-1">{renderTagline(p.tagline)}</p>
                )}
                <ul className="list-disc pl-4 mt-1 space-y-0.5 text-gray-600">
