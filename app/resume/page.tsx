...
const featuredProjects = [
  {
    ...
    github: "",
    tagline: "Animated gauge dashboards surfacing entry/exit market-timing signals generated by Quant Algo ZULFIQODE",
    bullets: [
...
async function downloadPDF() {
  ...
  featuredProjects.forEach(p => {
    ...
    if ("tagline" in p && p.tagline) {
      ...
      writeWrapped(p.tagline, M, CW, 3.8);
    }
  ...
  const children = [
    ...
    ...featuredProjects.flatMap((p) => [
      ...
      ...("tagline" in p && p.tagline
        ? [new Paragraph({ children: [new TextRun({ text: p.tagline, size: 18, color: "b45309", italics: true })] })]
        : []),
      ...p.bullets.map(
        (b) =>
          new Paragraph({
          ...
          <div className="space-y-4">
            {featuredProjects.map((p, i) => (
              <div key={i} className="text-sm">
                ...
                {"tagline" in p && p.tagline && (
                  <p className="text-xs italic text-amber-800 mt-1">{p.tagline}</p>
                )}
                <ul className="list-disc pl-4 mt-1 space-y-0.5 text-gray-600">
